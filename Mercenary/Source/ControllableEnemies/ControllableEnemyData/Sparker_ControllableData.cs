...
{
    public class Sparker_ControllableData : IEnemyControllable
    {
        ...
        private LogicSet castBLogic;
        private LogicBlock castBLogicBlock;
        private FieldInfo projectileAngleField;

        public Sparker_ControllableData()
        {
            this.enemyType = typeof(Enemy_Sparker_Basic);

            this.storedAnimSpeedField =
                this.enemyType.GetField("m_storedAnimSpeed",
                    BindingFlags.NonPublic | BindingFlags.GetField | BindingFlags.Instance);

            this.projectileAngleField =
                this.enemyType.GetField("m_projectileAngle",
                    BindingFlags.NonPublic | BindingFlags.GetField | BindingFlags.Instance);
        ...
        public void CastSpellB(CharacterObj source, PlayerObj playerObj)
        {
            ...
            float projectileTellDelay = 0.75f;
            float projectileDelay = 0.4f;
            int projectileLoop = 3;
            float projectileLoopDelay = 0.13f;
            float projectileExitDelay = 0.25f;

            int projectileAngle = (int) this.projectileAngleField.GetValue(source);

            if (this.castBLogic != null)
            {
                this.castBLogic?.Dispose();
                this.castBLogicBlock?.Dispose();
            }

            this.castBLogicBlock = new LogicBlock(RNG.seed);
            this.castBLogic = new LogicSet(this.GetType() + ":attackLogic");
            
            LogicSet.Begin(this.castBLogic);
            LogicSet.Add(new RunFunctionAction(this, "TurnToFace", new object[] {source}), SequenceType.Serial);
              ...
              LogicSet.Add(new RunFunctionAction(source, "EnableSparks", new object[]{}));
              LogicSet.Add(new DelayAction(projectileTellDelay - 0.1f));
              LogicSet.Add(new ChangeColourAction(source, Color.Red, 0.1f));
              LogicSet.Add(new DelayAction(projectileDelay));
              LogicSet.Add(new RunFunctionAction(source, "DisableSparks", new object[]{}));
              LogicSet.Add(new RunFunctionAction(source, "PlayPrepComplete", new object[]{}));
              for (int index = 0; index < projectileLoop; ++index)
              {
                LogicSet.Add(new RunFunctionAction(source, "FireProjectile", new object[1]
                {
                    0
                }));
                ...
                LogicSet.Add(new RunFunctionAction(source, "PlaySparkShotSFX", new object[]{}));
                LogicSet.Add(new DelayAction(projectileLoopDelay));
              }
              ...
              LogicSet.Add(new DelayAction(projectileExitDelay));
              LogicSet.Add(new ChangePropertyAction(source, "State", CharacterState.Idle));
              // LogicSet.Add((LogicAction) new CallCooldownAction((source);
              LogicSet.End();

              EnemyControllableManager.RunEnemyAttack((EnemyObj) source, ref this.castBLogic, ref this.castBLogicBlock);
        }

        public void CastSpellA(CharacterObj source, PlayerObj playerObj)
        {
            var logicSetField =
                this.enemyType.GetField("m_projectilePatternLogic",
                    BindingFlags.NonPublic | BindingFlags.GetField | BindingFlags.Instance);
        ...
        {
            var logicSetField =
                this.enemyType.GetField("m_projectileLogic",
                    BindingFlags.NonPublic | BindingFlags.GetField | BindingFlags.Instance);
